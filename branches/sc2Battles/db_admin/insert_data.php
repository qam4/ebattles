<?php
	...
	// Insert Ladders
	// Ladder 1 - 1v1 ladder test
	$query = 
	"INSERT INTO ".TBL_LADDERS."(Name,Password,Game,Type,Owner, Start_timestamp, End_timestamp, Rules, Description, AllowDraw, AllowScore)
	 VALUES ('1v1 Ladder Test', '', '1', 'One Player Ladder',1, $time, ($time+7*86400), '...', 
	 '<p style=\"text-align: center;\">This is a 1v1 test ladder.</p><p style=\"text-align: center;\">Feel free to join this ladder and use the \"Match Report\" system.</p><p style=\"text-align: center;\"><img src=\"http://www.visionfutur.com/img/linux/starcraft1.jpg\" border=\"0\" alt=\"Starcraft\" title=\"Starcraft\" width=\"640\" height=\"480\" /></p>',
	 1,1)";
	$sql->db_Query($query) or die ('Error, adding ladder 1<br />'. mysql_error());
	
    $last_id = mysql_insert_id();
 	$query = 
	"INSERT INTO ".TBL_STATSCATEGORIES."(Ladder, CategoryName, CategoryMinValue, CategoryMaxValue)
	...
	// Insert Mods in Ladder
	$query = 
	"INSERT INTO ".TBL_LADDERMODS."(Ladder,User,Level)
	 VALUES ($last_id,1,9)";
	$sql->db_Query($query) or die ('Error, adding moderator 1<br />'. mysql_error());
	$text .= "Added Mods<br />";

	// Insert Players in Ladder
	$query = 
	"INSERT INTO ".TBL_PLAYERS."(Ladder,User)
	 VALUES ($last_id,1)";
	...
	for ($i=1;$i<=50; $i++)
	{
	   ...
	   $sql->db_Query($query) or die ('Error, adding player '.$user_id.' in ladder '.$last_id.'<br />'. mysql_error());
	   $index++;
	}

    // Ladder 2 - team ladder test
	$query = 
	"INSERT INTO ".TBL_LADDERS."(Name,Password,Game,Type,Owner, Description)
	 VALUES ('Team Ladder Test', '', '1', 'Team Ladder',1, '<p>This is a Team test ladder.<br />Feel free to join this ladder and use the &quot;Match Report&quot; system. </p>')";
	$sql->db_Query($query) or die ('Error, adding ladder 3<br />'. mysql_error());
        $last_id = mysql_insert_id();
 	$query = 
	"INSERT INTO ".TBL_STATSCATEGORIES."(Ladder, CategoryName, CategoryMinValue, CategoryMaxValue)
	 VALUES ('$last_id', 'ELO', '1', '20')";
	...
	// Insert Teams in Ladder
	$query = 
	"INSERT INTO ".TBL_TEAMS."(Ladder,Division)
	 VALUES ($last_id,4)";
	...
	$query = 
	"INSERT INTO ".TBL_PLAYERS."(Ladder,User,Team)
	...
	// Ladder 3 - Old 1v1 ladder test
	$query = 
	"INSERT INTO ".TBL_LADDERS."(Name,Password,Game,Type,Owner, Start_timestamp, End_timestamp, Rules, Description)
	 VALUES ('Old 1v1 Ladder Test', '', '1', 'One Player Ladder',3, ($time-15*86400), ($time-7*86400), '...', 'This is a test past ladder')";
	$sql->db_Query($query) or die ('Error, adding ladder 2<br />'. mysql_error());
    $last_id = mysql_insert_id();
