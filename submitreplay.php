...
$text .= '
<form enctype="multipart/form-data" action="submitreplay.php" method="post">
			...
			if ($init == false)
				...
				foreach($players as $value) {
					...
					$text .= sprintf("<tr><td>%s</td><td>%s</td><td><font color=\"#%s\">%s</font></td><td>%s</td><td style=\"text-align: center\">%d</td><td style=\"background-color: #%06X; text-align: center\">%d</td></tr>\n",
									//$value['name'].'#'.$value['uid'].'@'.$value['uidIndex'].$difficultyString,
					...
					if (!$value['isObs'] && $value['ptype'] != 'Comp') {
						$apmFileName = 'tmp/'.$value['id']."_".md5($name).".png";
